...
3. Убедитесь, что в файле `.env` установлена переменная `ENCRYPTION_KEY` для обеспечения постоянного шифрования токенов:
```bash
ENCRYPTION_KEY=ваш_постоянный_ключ_шифрования
```

Примечание: Если переменная `ENCRYPTION_KEY` не установлена или изменяется, все ранее сохраненные токены станут недействительными.
